...
/* Set P=Q */
/* SU= 16 */
void BLS12381::ECP2_copy(ECP2 *P, ECP2 *Q)
{
    FP2_copy(&(P->x), &(Q->x));
    FP2_copy(&(P->y), &(Q->y));
    FP2_copy(&(P->z), &(Q->z));
...
static void ECP2_cmove(BLS12381::ECP2 *P, BLS12381::ECP2 *Q, int d)
{
    FP2_cmove(&(P->x), &(Q->x), d);
    FP2_cmove(&(P->y), &(Q->y), d);
    FP2_cmove(&(P->z), &(Q->z), d);
...
static void ECP2_select(BLS12381::ECP2 *P, BLS12381::ECP2 W[], sign32 b)
{
    BLS12381::ECP2 MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* SU= 312 */
int BLS12381::ECP2_equals(ECP2 *P, ECP2 *Q)
{
    FP2 a, b;
...
/* SU= 232 */
void BLS12381::ECP2_affine(ECP2 *P)
{
    FP2 one, iz;
...
/* SU= 16 */
int BLS12381::ECP2_get(FP2 *x, FP2 *y, ECP2 *P)
{
    ECP2 W;
    ECP2_copy(&W, P);
    ECP2_affine(&W);
...
/* SU= 152 */
/* Output point P */
void BLS12381::ECP2_output(ECP2 *P)
{
    FP2 x, y;
    if (ECP2_isinf(P))
    {
...
/* SU= 232 */
void BLS12381::ECP2_outputxyz(ECP2 *P)
{
    ECP2 Q;
    if (ECP2_isinf(P))
    {
...
void BLS12381::ECP2_toOctet(octet *W, ECP2 *Q, bool compress)
{
    FP2 qx, qy;
    bool alt=false;
    ECP2_get(&qx, &qy, Q);
    ...
    if (alt)
    {
        ...
        if (!compress)
        {
            W->len=4*MODBYTES_B384_29;
            FP2_toBytes(&(W->val[2*MODBYTES_B384_29]), &qy);
        } else {
            W->val[0]|=0x80;
        ...
        {
            W->val[0] = 0x04;
            FP2_toBytes(&(W->val[2 * MODBYTES_B384_29+1]), &qy);
            W->len = 4 * MODBYTES_B384_29 + 1;
        } else {
            W->val[0]=0x02;
...
int BLS12381::ECP2_fromOctet(ECP2 *Q, octet *W)
{
    FP2 qx, qy;
    bool alt=false;
    ...
    if (alt)
    {
        ...
        if ((typ&0x80)==0)
        {
            FP2_fromBytes(&qy,&(W->val[2*MODBYTES_B384_29]));
        ...
        if (typ == 0x04)
        {
            FP2_fromBytes(&qy,&(W->val[2 * MODBYTES_B384_29+1]));
            if (ECP2_set(Q, &qx, &qy)) return 1;
        } else {
...
void BLS12381::ECP2_rhs(FP2 *rhs, FP2 *x)
{
...
/* SU= 232 */
int BLS12381::ECP2_set(ECP2 *P, FP2 *x, FP2 *y)
{
    FP2 rhs, y2;
    FP2_sqr(&y2, y);
    ECP2_rhs(&rhs, x);
...
/* SU= 232 */
int BLS12381::ECP2_setx(ECP2 *P, FP2 *x, int s)
{
    FP2 y;
    FP hint;
    ECP2_rhs(&y, x);
...
/* Set P+=Q */
/* SU= 400 */
int BLS12381::ECP2_add(ECP2 *P, ECP2 *Q)
{
...
/* Set P-=Q */
/* SU= 16 */
void BLS12381::ECP2_sub(ECP2 *P, ECP2 *Q)
{
    ECP2 NQ;
    ECP2_copy(&NQ, Q);
    ECP2_neg(&NQ);
...
/* P*=e */
/* SU= 280 */
void BLS12381::ECP2_mul(ECP2 *P, BIG e)
{
    /* fixed size windows */
    int i, nb, s, ns;
    BIG mt, t;
...
/* SU= 96 */
void BLS12381::ECP2_frob(ECP2 *P, FP2 *X)
{
    FP2 X2;
    FP2_sqr(&X2, X);
    FP2_conj(&(P->x), &(P->x));
...
void BLS12381::ECP2_mul4(ECP2 *P, ECP2 Q[4], BIG u[4])
{
    int i, j, k, nb, pb, bt;
    ECP2 T[8], W;
    BIG t[4], mt;
    sign8 w[NLEN_B384_29 * BASEBITS_B384_29 + 1];
    sign8 s[NLEN_B384_29 * BASEBITS_B384_29 + 1];
...
/*
void BLS12381::ECP2_hap2point(ECP2 *Q,BIG h)
{
    BIG one,hv;
    FP2 X;
    BIG_one(one);
...
void BLS12381::ECP2_map2point(ECP2 *Q,FP2 *H)
{
// SSWU plus isogenies method
    ...
    FP2_from_ints(&ZZ,RIADZG2A_BLS12381,RIADZG2B_BLS12381);
...
// SVDW - Shallue and van de Woestijne method.
    ...
    FP_from_int(&Z,RIADZG2A_BLS12381);
    FP2_from_FP(&A,&Z);
    ECP2_rhs(&A,&A);  // A=g(Z)

    if (CURVE_B_I==4 && SEXTIC_TWIST_BLS12381==M_TYPE && RIADZG2A_BLS12381==-1 && RIADZG2B_BLS12381==0)
    ...
    FP2_from_ints(&X1,RIADZG2A_BLS12381,RIADZG2B_BLS12381);
    FP2_copy(&X3,&X1);
...
/*
void BLS12381::ECP2_mapit(ECP2 *Q, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
int BLS12381::ECP2_generator(ECP2 *G)
{
    FP2 wx, wy;
