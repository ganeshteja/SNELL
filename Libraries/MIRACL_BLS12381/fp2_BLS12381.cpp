...
/* Move g to f if d=1 */
void BLS12381::FP2_cmove(FP2 *f, FP2 *g, int d)
{
    FP_cmove(&(f->a), &(g->a), d);
    FP_cmove(&(f->b), &(g->b), d);
}
...
/* test x==1 ? */
/* SU= 48 */
int BLS12381::FP2_isunity(FP2 *x)
{
    FP one;
    FP_one(&one);
...
void BLS12381::FP2_reduce(FP2 *w)
{
    FP_reduce(&(w->a));
    FP_reduce(&(w->b));
}
...
/* SU= 16 */
int BLS12381::FP2_equals(FP2 *x, FP2 *y)
{
...
int BLS12381::FP2_islarger(FP2 *x)
{
    int cmp;
...
void BLS12381::FP2_toBytes(char *b,FP2 *x)
{
    FP_toBytes(b,&(x->b));
    FP_toBytes(&b[MODBYTES_B384_29],&(x->a));
}

void BLS12381::FP2_fromBytes(FP2 *x,char *b)
{
    FP_fromBytes(&(x->b),b);
    FP_fromBytes(&(x->a),&b[MODBYTES_B384_29]);
}

/* Create FP2 from two FPs */
/* SU= 16 */
void BLS12381::FP2_from_FPs(FP2 *w, FP *x, FP *y)
{
    FP_copy(&(w->a), x);
    FP_copy(&(w->b), y);
}
...
/* SU= 16 */
void BLS12381::FP2_from_BIGs(FP2 *w, BIG x, BIG y)
{
    FP_nres(&(w->a), x);
    FP_nres(&(w->b), y);
}
...
void BLS12381::FP2_from_ints(FP2 *w, int xa, int xb)
{
    FP a,b;
    FP_from_int(&a,xa);
    FP_from_int(&b,xb);
...
/* Create FP2 from FP */
/* SU= 8 */
void BLS12381::FP2_from_FP(FP2 *w, FP *x)
{
    FP_copy(&(w->a), x);
    FP_zero(&(w->b));
}
...
/* Create FP2 from BIG */
/* SU= 8 */
void BLS12381::FP2_from_BIG(FP2 *w, BIG x)
{
    FP_nres(&(w->a), x);
    FP_zero(&(w->b));
}
...
/* FP2 copy w=x */
/* SU= 16 */
void BLS12381::FP2_copy(FP2 *w, FP2 *x)
{
    if (w == x) return;
    FP_copy(&(w->a), &(x->a));
    FP_copy(&(w->b), &(x->b));
...
void BLS12381::FP2_rcopy(FP2 *w,const BIG a,const BIG b)
{
    FP_rcopy(&(w->a),a);
    FP_rcopy(&(w->b),b);
}

int BLS12381::FP2_sign(FP2 *w)
{
    int p1,p2;
    p1=FP_sign(&(w->a));
    p2=FP_sign(&(w->b));
...
/* Set w=-x */
/* SU= 88 */
void BLS12381::FP2_neg(FP2 *w, FP2 *x)
{
    /* Just one neg! */
    FP m, t;
    FP_add(&m, &(x->a), &(x->b));
...
/* Set w=conj(x) */
/* SU= 16 */
void BLS12381::FP2_conj(FP2 *w, FP2 *x)
{
    FP_copy(&(w->a), &(x->a));
    FP_neg(&(w->b), &(x->b));
    FP_norm(&(w->b));
...
/* Set w=x+y */
/* SU= 16 */
void BLS12381::FP2_add(FP2 *w, FP2 *x, FP2 *y)
{
    FP_add(&(w->a), &(x->a), &(y->a));
    FP_add(&(w->b), &(x->b), &(y->b));
}
...
void BLS12381::FP2_sub(FP2 *w, FP2 *x, FP2 *y)
{
    FP2 m;
    FP2_neg(&m, y);
    FP2_add(w, x, &m);
...
/* SU= 16 */
void BLS12381::FP2_pmul(FP2 *w, FP2 *x, FP *s)
{
    FP_mul(&(w->a), &(x->a), s);
    FP_mul(&(w->b), &(x->b), s);
}
...
void BLS12381::FP2_imul(FP2 *w, FP2 *x, int s)
{
    FP_imul(&(w->a), &(x->a), s);
    FP_imul(&(w->b), &(x->b), s);
}
...
/* Set w=x^2 */
/* SU= 128 */
void BLS12381::FP2_sqr(FP2 *w, FP2 *x)
{
    FP w1, w3, mb;
...
void BLS12381::FP2_mul(FP2 *w, FP2 *x, FP2 *y)
{
    DBIG A, B, E, F, pR;
    BIG C, D, p;
...
/* SU= 16 */
void BLS12381::FP2_output(FP2 *w)
{
    BIG bx, by;
    FP2_reduce(w);
    FP_redc(bx, &(w->a));
...
/* SU= 8 */
void BLS12381::FP2_rawoutput(FP2 *w)
{
    printf("[");
    BIG_rawoutput(w->a.g);
    printf(",");
...
/* Set w=1/x */
/* SU= 128 */
void BLS12381::FP2_inv(FP2 *w, FP2 *x, FP *h)
{
    FP w1, w2;
...
/* Set w=x/2 */
/* SU= 16 */
void BLS12381::FP2_div2(FP2 *w, FP2 *x)
{
    FP_div2(&(w->a), &(x->a));
    FP_div2(&(w->b), &(x->b));
}
...
void BLS12381::FP2_times_i(FP2 *w)
{
    FP z;
    FP_copy(&z, &(w->a));
    FP_neg(&(w->a), &(w->b));
...
/* Input MUST be normed */
void BLS12381::FP2_mul_ip(FP2 *w)
{
    FP2 t;
    int i = QNRI_BLS12381;
...
/* Set w/=(2^i+sqrt(-1)) */
/* Slow */
void BLS12381::FP2_div_ip(FP2 *w)
{
    FP2 z;
    FP2_norm(w);
    FP2_from_ints(&z, (1 << QNRI_BLS12381), 1);
    FP2_inv(&z, &z, NULL);
    FP2_mul(w, &z, w);
#if TOWER_BLS12381 == POSITOWER
    FP2_neg(w, w);  // ***
...
/* SU= 208 */
/*
void BLS12381::FP2_pow(FP2 *r, FP2* a, BIG b)
{
    FP2 w;
    FP one;
    BIG z, zilch;
...
/* test for x a QR */
int BLS12381::FP2_qr(FP2 *x,FP *h)
{ /* test x^(p^2-1)/2 = 1 */
...
void BLS12381::FP2_sqrt(FP2 *w, FP2 *u, FP *h)
{
    FP w1, w2, w3, w4, hint;
    FP2 nw;
    int sgn,qr;
...
void BLS12381::FP2_rand(FP2 *x,csprng *rng)
{
    FP_rand(&(x->a),rng);
    FP_rand(&(x->b),rng);
}
