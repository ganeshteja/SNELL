...
/* Encode octet to curve point */
void BLS12381::MPIN_ENCODE_TO_CURVE(octet *DST,octet *ID,octet *HCID)
{
    int j,k,m,L;
    char okm[512],fd[256];
    BIG q,r,w;
...
int BLS12381::MPIN_RANDOM_GENERATE(csprng *RNG, octet* S)
{
    BIG r, s;
    BIG_rcopy(r, CURVE_Order);
    BIG_randtrunc(s, r, 2 * CURVE_SECURITY_BLS12381, RNG);
    BIG_toBytes(S->val, s);
    S->len = MODBYTES_B384_29;
    return 0;
}

/* Extract PIN from TOKEN for identity CID */
int BLS12381::MPIN_EXTRACT_PIN(octet *CID, int pin, octet *TOKEN)
{
    ECP P, R;
    int res = 0;
    pin %= MAXPIN;
...
int BLS12381::MPIN_CLIENT_2(octet *X, octet *Y, octet *SEC)
{
    BIG px, py, r;
    ECP P;
    int res = 0;
...
int BLS12381::MPIN_GET_CLIENT_SECRET(octet *S, octet *IDHTC, octet *CST)
{
    ECP P;
    BIG s;
    int res = 0;
...
int BLS12381::MPIN_CLIENT_1(octet *CID, csprng *RNG, octet *X, int pin, octet *TOKEN, octet *SEC, octet *xID)
{
    BIG r, x;
    ECP P, T, W;
    int res = 0;
    ...
    if (RNG != NULL)
    {
        BIG_randtrunc(x, r, 2 * CURVE_SECURITY_BLS12381, RNG);
        X->len = MODBYTES_B384_29;
        BIG_toBytes(X->val, x);
    }
    else
...
int BLS12381::MPIN_GET_SERVER_SECRET(octet *S, octet *SST)
{
    BIG r, s;
    ECP2 Q;
    int res = 0;
    ...
    if (res == 0)
    {
        BIG_fromBytes(s, S->val);
        PAIR_G2mul(&Q, s);
        ECP2_toOctet(SST, &Q, false);
    }
    return res;
}

/* Implement M-Pin on server side */
int BLS12381::MPIN_SERVER(octet *HID, octet *Y, octet *SST, octet *xID, octet *mSEC)
{
    BIG y;
    FP12 g;
    ECP2 Q, sQ;
