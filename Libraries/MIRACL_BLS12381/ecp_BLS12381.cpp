...
static void ECP_cswap(BLS12381::ECP *P, BLS12381::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_BLS12381!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(BLS12381::ECP *P, BLS12381::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_BLS12381!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
static void ECP_select(BLS12381::ECP *P, BLS12381::ECP W[], sign32 b)
{
    BLS12381::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int BLS12381::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
    FP_mul(&a, &(P->x), &(Q->z));
    FP_mul(&b, &(Q->x), &(P->z));
...
/* Set P=Q */
/* SU=16 */
void BLS12381::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_BLS12381!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
/* SU=56 */
void BLS12381::ECP_rhs(FP *v, FP *x)
{
#if CURVETYPE_BLS12381==WEIERSTRASS
    /* x^3+Ax+B */
    FP t;
    ...
    FP_mul(&x3, &x2, x);
    FP_copy(v, x);
    FP_imul(&x2, &x2, CURVE_A_BLS12381);
    FP_add(v, v, &x2);
    FP_add(v, v, &x3);
    FP_reduce(v);
#endif
...
int BLS12381::ECP_set(ECP *P, BIG x)
{
    FP rhs;
    FP_nres(&rhs, x);
...
int BLS12381::ECP_get(BIG x, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* SU=16 */
int BLS12381::ECP_get(BIG x, BIG y, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* Set P=(x,{y}) */
/* SU=96 */
int BLS12381::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int BLS12381::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    FP_nres(&rhs, x);
...
/* SU=160 */
void BLS12381::ECP_affine(ECP *P)
{
    FP one, iz;
...
/* SU=120 */
void BLS12381::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void BLS12381::ECP_output(ECP *P)
{
    BIG x, y;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void BLS12381::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void BLS12381::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_BLS12381==MONTGOMERY
    BIG x;
    ECP_get(x, P);
    W->len = MODBYTES_B384_29; // + 1;
    ...
    if (alt)
    {
        ...
        if (compress)
        {
            W->len = MODBYTES_B384_29;
            W->val[0]|=0x80;
            if (FP_islarger(&(P->y))==1) W->val[0]|=0x20;
        } else {
            W->len = 2 * MODBYTES_B384_29;
            BIG_toBytes(&(W->val[MODBYTES_B384_29]), y);
        }
    } else {
        ...
        if (compress)
        {
            ...
            W->val[0] = 0x04;
            W->len = 2 * MODBYTES_B384_29 + 1;
            BIG_toBytes(&(W->val[MODBYTES_B384_29 + 1]), y);
        }
    }
#endif
}
...
int BLS12381::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_BLS12381==MONTGOMERY
    BIG x;
    ...
    if (alt)
    {
        ...
        if ((typ&0x80)==0)
        {
            BIG_fromBytes(y, &(W->val[MODBYTES_B384_29]));
        ...
        if (typ == 0x04)
        {
            BIG_fromBytes(y, &(W->val[MODBYTES_B384_29 + 1]));
            if (ECP_set(P, x, y)) return 1;
        }
...
void BLS12381::ECP_dbl(ECP *P)
{
    ...
    FP_imul(&A, &C, (CURVE_A_BLS12381 + 2) / 4); //A.imul((ROM.CURVE_A+2)/4);
...
void BLS12381::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void BLS12381::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_BLS12381==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  BLS12381::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void BLS12381::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
void BLS12381::ECP_mul(ECP *P,BIG e)
{
    ECP_clmul(P,e,e);
}
...
// Set P=e*P 
void BLS12381::ECP_clmul(ECP *P, BIG e, BIG maxe)
{
    BIG cm;
    BIG_or(cm,e,maxe);
...
void BLS12381::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
...
void BLS12381::ECP_clmul2(ECP *P, ECP *Q, BIG e, BIG f, BIG maxe)
{
    BIG cm;
    BIG te, tf, mt;
    ECP S, T, W[8], C;
    sign8 w[1 + (NLEN_B384_29 * BASEBITS_B384_29 + 1) / 2];
    int i, a, b, s, ns, nb;
...
// Elligator 2
    ...
    FP_from_int(&A,CURVE_A_BLS12381);
    FP_copy(&t,h);
    FP_sqr(&t,&t);   // t^2
    ...
    if (PM1D2_BLS12381 == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_BLS12381 > 2)
        FP_imul(&t,&t,QNRI_BLS12381); // precomputed QNR
    FP_norm(&t);  // z.t^2
...
// Map to point on isogenous curve
    ...
    sgn=FP_sign(&t);
        ...
        FP_sqr(&t,&t);
        FP_imul(&t,&t,RIADZ_BLS12381);  // Z from hash-to-point draft standard
...
// SVDW - Shallue and van de Woestijne
        FP_from_int(&Y,RIADZ_BLS12381);
        ECP_rhs(&A,&Y);  // A=g(Z)
        FP_rcopy(&B,SQRTm3);
        FP_imul(&B,&B,RIADZ_BLS12381); // B=Z*sqrt(-3)
...
/*
void BLS12381::ECP_hap2point(ECP *P,BIG h)
{
    BIG x;
    BIG_copy(x,h);
	for (;;)
...
/* Map octet to point */
/*
void BLS12381::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
*/
int BLS12381::ECP_generator(ECP *G)
{
    BIG x, y;
