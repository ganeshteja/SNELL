...
/* So when multiplying two numbers, their product *must* be less than MODBITS_BLS12381+BASEBITS_B384_29*NLEN_B384_29 */
/* Results *may* be one bit bigger than MODBITS_BLS12381 */
...
void BLS12381::FP_redc(BIG x, FP *y)
{
    BIG_copy(x, y->g);
}

/* reduce a DBIG to a BIG exploiting the special form of the modulus */
void BLS12381::FP_mod(BIG r, DBIG d)
{
    BIG t, b;
    chunk v, tw;
    BIG_split(t, b, d, MODBITS_BLS12381);
    ...
    if (MConst < NEXCESS_B384_29)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B384_29 - 1];
        r[NLEN_B384_29 - 1] &= TMASK_BLS12381;
        r[0] += MConst * ((tw >> TBITS_BLS12381));
    }
    else
    {
        ...
        r[NLEN_B384_29 - 1] &= TMASK_BLS12381;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_BLS12381) + (v << (BASEBITS_B384_29 - TBITS_BLS12381))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_BLS12381) + (v << (BASEBITS_B384_29 - TBITS_BLS12381)));
#endif
    }
    BIG_norm(r);
}
...
void BLS12381::FP_mod(BIG r, DBIG d)
{
    ...
    BIG t, b;
    chunk carry;
    BIG_split(t, b, d, MBITS_BLS12381);

    BIG_add(r, t, b);

    BIG_dscopy(d, t);
    BIG_dshl(d, MBITS_BLS12381 / 2);
    ...
    BIG_add(r, r, b);
    BIG_norm(r);
    BIG_shl(t, MBITS_BLS12381 / 2);

    BIG_add(r, r, t);

    carry = r[NLEN_B384_29 - 1] >> TBITS_BLS12381;

    r[NLEN_B384_29 - 1] &= TMASK_BLS12381;
    r[0] += carry;

    r[224 / BASEBITS_B384_29] += carry << (224 % BASEBITS_B384_29); /* need to check that this falls mid-word */
    BIG_norm(r);

/*

    BIG t, b, t2, b2;
    int BTset = MBITS_BLS12381 / 2;
    chunk carry;
    BIG_split(t, b, d, MBITS_BLS12381);
    ...
    BIG_split(t2, b2, d, MBITS_BLS12381);
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B384_29 - 1] >> TBITS_BLS12381; // + (carry<<(BASEBITS_B384_29-TBITS_BLS12381));
    r[NLEN_B384_29 - 1] &= TMASK_BLS12381;

    r[BTset / BASEBITS_B384_29] += carry << (BTset % BASEBITS_B384_29); // need to check that this falls mid-word 
...
void BLS12381::FP_mod(BIG a, DBIG d)
{
    int i;

    for (i = 0; i < NLEN_B384_29; i++)
        d[NLEN_B384_29 + i] += muladd(d[i], MConst - 1, d[i], &d[NLEN_B384_29 + i - 1]);
...
void BLS12381::FP_from_int(FP *x,int a)
{
    BIG w;
...
void BLS12381::FP_rcopy(FP *y, const BIG c)
{
    BIG b;
    BIG_rcopy(b, c);
    FP_nres(y, b);
...
/* Swap a and b if d=1 */
void BLS12381::FP_cswap(FP *a, FP *b, int d)
{
    sign32 t, c = d;
    BIG_cswap(a->g, b->g, d);
...
/* Move b to a if d=1 */
void BLS12381::FP_cmove(FP *a, FP *b, int d)
{
    sign32 c = -d;
...
void BLS12381::FP_toBytes(char *b,FP *x)
{
    BIG t;
    FP_redc(t, x);
    BIG_toBytes(b, t);
...
void BLS12381::FP_fromBytes(FP *x,char *b)
{
    BIG t;
    BIG_fromBytes(t, b);
    FP_nres(x, t);
...
void BLS12381::FP_rawoutput(FP *r)
{
    BIG_rawoutput(r->g);
}
...
/* SU= 136 */
void BLS12381::FP_imul(FP *r, FP *a, int c)
{
    DBIG d;
    BIG k;
    FP f;
...
/* Set r=a^2 mod m */
/* SU= 88 */
void BLS12381::FP_sqr(FP *r, FP *a)
{
    DBIG d;
...
/* SU= 16 */
/* Set r=a+b */
void BLS12381::FP_add(FP *r, FP *a, FP *b)
{
    BIG_add(r->g, a->g, b->g);
    r->XES = a->XES + b->XES;
...
/* Set r=a-b mod m */
/* SU= 56 */
void BLS12381::FP_sub(FP *r, FP *a, FP *b)
{
    FP n;
    FP_neg(&n, b);
    FP_add(r, a, &n);
...
static int quo(BIG n, BIG m)
{
    ...
    if (TBITS_BLS12381 < hb)
    {
        ...
        num = (n[NLEN_B384_29 - 1] << sh) | (n[NLEN_B384_29 - 2] >> (BASEBITS_B384_29 - sh));
        den = (m[NLEN_B384_29 - 1] << sh) | (m[NLEN_B384_29 - 2] >> (BASEBITS_B384_29 - sh));
    }
    else
    {
        num = n[NLEN_B384_29 - 1];
        den = m[NLEN_B384_29 - 1];
    }
...
void BLS12381::FP_reduce(FP *a)
{
    ...
    if (a->XES > 16)
    {
        ...
        r[NLEN_B384_29 - 1] += (carry << BASEBITS_B384_29); // correction - put any carry out back in again
        BIG_sub(a->g, a->g, r);
        BIG_norm(a->g);
        sb = 2;
    }
...
/* Set r=-a mod Modulus */
/* SU= 64 */
void BLS12381::FP_neg(FP *r, FP *a)
{
    int sb;
    BIG m;
...
/* Set r=a/2. */
/* SU= 56 */
void BLS12381::FP_div2(FP *r, FP *a)
{
    BIG m;
    BIG w;
    BIG_rcopy(m, Modulus);
...
void BLS12381::FP_pow(FP *r, FP *a, BIG b)
{
    sign8 w[1 + (NLEN_B384_29 * BASEBITS_B384_29 + 3) / 4];
    FP tb[16];
    BIG t;
    int i, nb;
...
void BLS12381::FP_fpow(FP *r, FP *x)
{
    int i, j, k, bw, w, nw, lo, m, n, c, nd, e=PM1D2_BLS12381;
    FP xp[11], t, key;
...
void BLS12381::FP_progen(FP *r,FP *x)
{
#if MODTYPE_BLS12381==PSEUDO_MERSENNE  || MODTYPE_BLS12381==GENERALISED_MERSENNE
    FP_fpow(r, x);  
#else
...
/* Modular inversion */
void BLS12381::FP_inv(FP *r,FP *x,FP *h)
{
    int i,e=PM1D2_BLS12381;
    FP s,t;
    FP_norm(x);
...
void BLS12381::FP_sqrt(FP *r, FP *a, FP* h)
{
...
int BLS12381::FP_invsqrt(FP *i, FP *s, FP *x)
{
    FP h;
    int qr=FP_qr(x,&h);
    FP_sqrt(s,x,&h);
...
void BLS12381::FP_rand(FP *x,csprng *rng)
{
    BIG w,m;
    BIG_rcopy(m,Modulus);
