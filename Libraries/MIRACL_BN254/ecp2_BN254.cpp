...
void BN254::ECP2_toOctet(octet *W, ECP2 *Q, bool compress)
{
    ...
    if (alt)
    {
        ...
        if (!compress)
        {
            W->len=4*MODBYTES_B256_28;
            FP2_toBytes(&(W->val[2*MODBYTES_B256_28]), &qy);
        } else {
            W->val[0]|=0x80;
        ...
        {
            W->val[0] = 0x04;
            FP2_toBytes(&(W->val[2 * MODBYTES_B256_28+1]), &qy);
...
int BN254::ECP2_fromOctet(ECP2 *Q, octet *W)
{
    ...
    if (alt)
    {
        ...
        if ((typ&0x80)==0)
        {
            FP2_fromBytes(&qy,&(W->val[2*MODBYTES_B256_28]));
        ...
        if (typ == 0x04)
        {
            FP2_fromBytes(&qy,&(W->val[2 * MODBYTES_B256_28+1]));
            if (ECP2_set(Q, &qx, &qy)) return 1;
        } else {
...
// SSWU plus isogenies method
    ...
    FP2_from_ints(&ZZ,RIADZG2A_BN254,RIADZG2B_BN254);
...
// SVDW - Shallue and van de Woestijne method.
    ...
    FP_from_int(&Z,RIADZG2A_BN254);
    FP2_from_FP(&A,&Z);
    ECP2_rhs(&A,&A);  // A=g(Z)

    if (CURVE_B_I==4 && SEXTIC_TWIST_BN254==M_TYPE && RIADZG2A_BN254==-1 && RIADZG2B_BN254==0)
    ...
    FP2_from_ints(&X1,RIADZG2A_BN254,RIADZG2B_BN254);
    FP2_copy(&X3,&X1);
