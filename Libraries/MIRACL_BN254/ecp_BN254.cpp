...
static void ECP_cswap(BN254::ECP *P, BN254::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_BN254!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(BN254::ECP *P, BN254::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_BN254!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
/* Test P == Q */
/* SU=168 */
int BN254::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
    FP_mul(&a, &(P->x), &(Q->z));
    FP_mul(&b, &(Q->x), &(P->z));
...
/* SU=136 */
int BN254::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    FP_nres(&rhs, x);
...
/* SU=120 */
void BN254::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void BN254::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void BN254::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_BN254==MONTGOMERY
    BIG x;
    ECP_get(x, P);
    W->len = MODBYTES_B256_28; // + 1;
    ...
    if (alt)
    {
        ...
        if (compress)
        {
            W->len = MODBYTES_B256_28;
            W->val[0]|=0x80;
            if (FP_islarger(&(P->y))==1) W->val[0]|=0x20;
        } else {
            W->len = 2 * MODBYTES_B256_28;
            BIG_toBytes(&(W->val[MODBYTES_B256_28]), y);
        }
    } else {
        ...
        if (compress)
        {
            ...
            BIG_toBytes(&(W->val[MODBYTES_B256_28 + 1]), y);
        }
    }
#endif
}
...
int BN254::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_BN254==MONTGOMERY
    BIG x;
    ...
    if (alt)
    {
        ...
        if ((typ&0x80)==0)
        {
            BIG_fromBytes(y, &(W->val[MODBYTES_B256_28]));
        ...
        if (typ == 0x04)
        {
            BIG_fromBytes(y, &(W->val[MODBYTES_B256_28 + 1]));
            if (ECP_set(P, x, y)) return 1;
        }
...
void BN254::ECP_dbl(ECP *P)
{
    ...
    FP_imul(&A, &C, (CURVE_A_BN254 + 2) / 4); //A.imul((ROM.CURVE_A+2)/4);
...
void BN254::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
void BN254::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
void BN254::ECP_mul(ECP *P,BIG e)
{
    ECP_clmul(P,e,e);
}
...
// m point doublings
void BN254::ECP_muln(ECP *P,int n,ECP X[],BIG e[])
{
    int i,j,k,nb;
    BIG t,mt;
    ECP S,R,B[16];
...
void BN254::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
...
void BN254::ECP_clmul2(ECP *P, ECP *Q, BIG e, BIG f, BIG maxe)
{
    BIG cm;
    BIG te, tf, mt;
    ECP S, T, W[8], C;
    sign8 w[1 + (NLEN_B256_28 * BASEBITS_B256_28 + 1) / 2];
    int i, a, b, s, ns, nb;
...
void BN254::ECP_map2point(ECP *P,FP *h)
{
#if CURVETYPE_BN254==MONTGOMERY
// Elligator 2
    int qres;
    ...
    if (PM1D2_BN254 == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_BN254 > 2)
        FP_imul(&t,&t,QNRI_BN254); // precomputed QNR
    FP_norm(&t);  // z.t^2
...
// Map to point on isogenous curve
    ...
    sgn=FP_sign(&t);
        ...
        FP_sqr(&t,&t);
        FP_imul(&t,&t,RIADZ_BN254);  // Z from hash-to-point draft standard
...
// SVDW - Shallue and van de Woestijne
        FP_from_int(&Y,RIADZ_BN254);
        ECP_rhs(&A,&Y);  // A=g(Z)
        FP_rcopy(&B,SQRTm3);
        FP_imul(&B,&B,RIADZ_BN254); // B=Z*sqrt(-3)
...
void BN254::ECP_hap2point(ECP *P,BIG h)
{
    BIG x;
    BIG_copy(x,h);
	for (;;)
...
/* Map octet to point */
/*
void BN254::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
