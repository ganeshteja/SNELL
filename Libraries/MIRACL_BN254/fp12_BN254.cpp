...
static void FP12_select(BN254::FP12 *f, BN254::FP12 g[], sign32 b)
{
    BN254::FP12 invf;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* FP12 copy w=x */
/* SU= 16 */
void BN254::FP12_copy(FP12 *w, FP12 *x)
{
    if (x == w) return;
    FP4_copy(&(w->a), &(x->a));
    FP4_copy(&(w->b), &(x->b));
...
/* SU= 16 */
int BN254::FP12_equals(FP12 *x, FP12 *y)
{
...
/* Set w=conj(x) */
/* SU= 8 */
void BN254::FP12_conj(FP12 *w, FP12 *x)
{
    FP12_copy(w, x);
    FP4_conj(&(w->a), &(w->a));
    FP4_nconj(&(w->b), &(w->b));
...
/* SU= 8 */
void BN254::FP12_from_FP4(FP12 *w, FP4 *a)
{
    FP4_copy(&(w->a), a);
    FP4_zero(&(w->b));
    FP4_zero(&(w->c));
...
/* SU= 16 */
void BN254::FP12_from_FP4s(FP12 *w, FP4 *a, FP4 *b, FP4 *c)
{
    FP4_copy(&(w->a), a);
    FP4_copy(&(w->b), b);
    FP4_copy(&(w->c), c);
...
/* SU= 600 */
void BN254::FP12_usqr(FP12 *w, FP12 *x)
{
    FP4 A, B, C, D;
...
/* FP12 squaring w=x^2 */
/* SU= 600 */
void BN254::FP12_sqr(FP12 *w, FP12 *x)
{
...
void BN254::FP12_mul(FP12 *w, FP12 *y)
{
...
void BN254::FP12_ssmul(FP12 *w, FP12 *y)
{
    FP4 z0, z1, z2, z3, t0, t1;
    if (w->type == FP_UNITY)
    {
...
void BN254::FP12_smul(FP12 *w, FP12 *y)
{
...
/* Set w=1/x */
/* SU= 600 */
void BN254::FP12_inv(FP12 *w, FP12 *x)
{
    FP4 f0, f1, f2, f3;
...
void BN254::FP12_pinpow(FP12 *r, int e, int bts)
{
    int i, b;
    FP12 R[2];
...
void BN254::FP12_compow(FP4 *c, FP12 *x, BIG e, BIG r)
{
    FP12 g1, g2;
    FP4 cp, cpm1, cpm2;
    FP2 f;
...
void BN254::FP12_pow(FP12 *r, FP12 *a, BIG b)
{
    FP12 w, sf;
    BIG b1, b3;
    int i, nb, bt;
...
void BN254::FP12_pow4(FP12 *p, FP12 *q, BIG u[4])
{
    int i, j, k, nb, pb, bt;
    FP12 g[8], r;
    BIG t[4], mt;
...
/* SU= 160 */
void BN254::FP12_frob(FP12 *w, FP2 *f)
{
    FP2 f2, f3;
    FP2_sqr(&f2, f);    /* f2=f^2 */
...
/* SU= 8 */
void BN254::FP12_trace(FP4 *w, FP12 *x)
{
    FP4_imul(w, &(x->a), 3);
    FP4_reduce(w);
}
...
void BN254::FP12_toOctet(octet *W, FP12 *g)
{
    W->len = 12 * MODBYTES_B256_28;
    FP4_toBytes(&(W->val[0]),&(g->c));
    FP4_toBytes(&(W->val[4 * MODBYTES_B256_28]),&(g->b));
    FP4_toBytes(&(W->val[8 * MODBYTES_B256_28]),&(g->a));
}

/* SU= 24 */
/* Restore g from octet string w */
void BN254::FP12_fromOctet(FP12 *g, octet *W)
{
    FP4_fromBytes(&(g->c),&(W->val[0]));
    FP4_fromBytes(&(g->b),&(W->val[4 * MODBYTES_B256_28]));
    FP4_fromBytes(&(g->a),&(W->val[8 * MODBYTES_B256_28]));

}

/* Move g to f
if d=1 */
void BN254::FP12_cmove(FP12 *f, FP12 *g, int d)
{
    FP4_cmove(&(f->a), &(g->a), d);
    FP4_cmove(&(f->b), &(g->b), d);
    FP4_cmove(&(f->c), &(g->c), d);
