...
/* test x==1 ? */
/* SU= 8 */
int BN254::FP4_isunity(FP4 *x)
{
    return (FP2_isunity(&(x->a)) & FP2_iszilch(&(x->b)));
}

/* test is w real? That is in a+ib test b is zero */
int BN254::FP4_isreal(FP4 *w)
{
...
void BN254::FP4_toBytes(char *b,FP4 *x)
{
    FP2_toBytes(b,&(x->b));
    FP2_toBytes(&b[2*MODBYTES_B256_28],&(x->a));
}

void BN254::FP4_fromBytes(FP4 *x,char *b)
{
    FP2_fromBytes(&(x->b),b);
    FP2_fromBytes(&(x->a),&b[2*MODBYTES_B256_28]);
}

/* return 1 if x==y, else 0 */
/* SU= 16 */
int BN254::FP4_equals(FP4 *x, FP4 *y)
{
...
/* SU= 16 */
void BN254::FP4_from_FP2s(FP4 *w, FP2 * x, FP2* y)
{
    FP2_copy(&(w->a), x);
    FP2_copy(&(w->b), y);
}
...
/* set FP4 from FP2 */
/* SU= 8 */
void BN254::FP4_from_FP2(FP4 *w, FP2 *x)
{
    FP2_copy(&(w->a), x);
    FP2_zero(&(w->b));
}
...
/* SU= 8 */
void BN254::FP4_from_FP2H(FP4 *w, FP2 *x)
{
    FP2_copy(&(w->b), x);
    FP2_zero(&(w->a));
}
...
/* set FP4 from FP */
void BN254::FP4_from_FP(FP4 *w, FP *x)
{
    FP2 t;
    FP2_from_FP(&t, x);
    FP4_from_FP2(w, &t);
...
/* FP4 copy w=x */
/* SU= 16 */
void BN254::FP4_copy(FP4 *w, FP4 *x)
{
    if (w == x) return;
    FP2_copy(&(w->a), &(x->a));
    FP2_copy(&(w->b), &(x->b));
...
/* FP4 w=1 */
/* SU= 8 */
void BN254::FP4_one(FP4 *w)
{
    FP2_one(&(w->a));
    FP2_zero(&(w->b));
}

int BN254::FP4_sign(FP4 *w)
{
    int p1,p2;
    p1=FP2_sign(&(w->a));
    p2=FP2_sign(&(w->b));
...
/* Set w=-x */
/* SU= 160 */
void BN254::FP4_neg(FP4 *w, FP4 *x)
{
    /* Just one field neg */
    FP2 m, t;
    FP4_norm(x);
...
/* Set w=conj(x) */
/* SU= 16 */
void BN254::FP4_conj(FP4 *w, FP4 *x)
{
    FP2_copy(&(w->a), &(x->a));
    FP2_neg(&(w->b), &(x->b));
    FP4_norm(w);
...
/* Set w=-conj(x) */
/* SU= 16 */
void BN254::FP4_nconj(FP4 *w, FP4 *x)
{
    FP2_copy(&(w->b), &(x->b));
    FP2_neg(&(w->a), &(x->a));
    FP4_norm(w);
...
/* Set w=x+y */
/* SU= 16 */
void BN254::FP4_add(FP4 *w, FP4 *x, FP4 *y)
{
    FP2_add(&(w->a), &(x->a), &(y->a));
    FP2_add(&(w->b), &(x->b), &(y->b));
}
...
void BN254::FP4_sub(FP4 *w, FP4 *x, FP4 *y)
{
    FP4 my;
    FP4_neg(&my, y);
    FP4_add(w, x, &my);
...
/* SU= 16 */
void BN254::FP4_pmul(FP4 *w, FP4 *x, FP2 *s)
{
    FP2_mul(&(w->a), &(x->a), s);
    FP2_mul(&(w->b), &(x->b), s);
}
...
void BN254::FP4_qmul(FP4 *w, FP4 *x, FP *s)
{
    FP2_pmul(&(w->a), &(x->a), s);
    FP2_pmul(&(w->b), &(x->b), s);
}
...
void BN254::FP4_imul(FP4 *w, FP4 *x, int s)
{
    FP2_imul(&(w->a), &(x->a), s);
    FP2_imul(&(w->b), &(x->b), s);
}
...
void BN254::FP4_sqr(FP4 *w, FP4 *x)
{
    FP2 t1, t2, t3;
...
void BN254::FP4_mul(FP4 *w, FP4 *x, FP4 *y)
{
...
/* SU= 8 */
void BN254::FP4_rawoutput(FP4 *w)
{
    printf("[");
    FP2_rawoutput(&(w->a));
    printf(",");
...
/* Set w=1/x */
/* SU= 160 */
void BN254::FP4_inv(FP4 *w, FP4 *x, FP *h)
{
    FP2 t1, t2;
    FP2_sqr(&t1, &(x->a));
    FP2_sqr(&t2, &(x->b));
...
/* SU= 16 */
void BN254::FP4_frob(FP4 *w, FP2 *f)
{
    FP2_conj(&(w->a), &(w->a));
    FP2_conj(&(w->b), &(w->b));
    FP2_mul( &(w->b), f, &(w->b));
...
/* SU= 240 */
/*
void BN254::FP4_pow(FP4 *r, FP4* a, BIG b)
{
    FP4 w;
    BIG z, zilch;
    int bt;
...
/* SU= 304 */
/* XTR xtr_a function */
void BN254::FP4_xtr_A(FP4 *r, FP4 *w, FP4 *x, FP4 *y, FP4 *z)
{
    FP4 t1, t2;
...
/* SU= 152 */
/* XTR xtr_d function */
void BN254::FP4_xtr_D(FP4 *r, FP4 *x)
{
    FP4 w;
    FP4_copy(r, x);
    FP4_conj(&w, r);
...
void BN254::FP4_xtr_pow(FP4 *r, FP4 *x, BIG n)
{
    int i, par, nb;
    BIG v;
    FP2 w;
...
void BN254::FP4_xtr_pow2(FP4 *r, FP4 *ck, FP4 *cl, FP4 *ckml, FP4 *ckm2l, BIG a, BIG b)
{
    int i, f2;
    BIG d, e, w;
...
/* Set w=x/2 */
void BN254::FP4_div2(FP4 *w, FP4 *x)
{
    FP2_div2(&(w->a), &(x->a));
    FP2_div2(&(w->b), &(x->b));
}
...
/* Move b to a if d=1 */
void BN254::FP4_cmove(FP4 *f, FP4 *g, int d)
{
    FP2_cmove(&(f->a), &(g->a), d);
    FP2_cmove(&(f->b), &(g->b), d);
}

void BN254::FP4_rand(FP4 *x,csprng *rng)
{
    FP2_rand(&(x->a),rng);
    FP2_rand(&(x->b),rng);
}
...
void BN254::FP4_sqrt(FP4 *r, FP4* x, FP *h)
{
    FP2 a, b, s, t;
    FP hint,twk;
    FP4 nr;
