...
void BN254::FP_mod(BIG r, DBIG d)
{
    BIG t, b;
    chunk v, tw;
    BIG_split(t, b, d, MODBITS_BN254);
    ...
    if (MConst < NEXCESS_B256_28)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B256_28 - 1];
        r[NLEN_B256_28 - 1] &= TMASK_BN254;
        r[0] += MConst * ((tw >> TBITS_BN254));
    }
    else
    {
        ...
        r[NLEN_B256_28 - 1] &= TMASK_BN254;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_BN254) + (v << (BASEBITS_B256_28 - TBITS_BN254))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_BN254) + (v << (BASEBITS_B256_28 - TBITS_BN254)));
#endif
    }
    BIG_norm(r);
}
...
void BN254::FP_mod(BIG r, DBIG d)
{
    ...
    BIG t, b;
    chunk carry;
    BIG_split(t, b, d, MBITS_BN254);

    BIG_add(r, t, b);

    BIG_dscopy(d, t);
    BIG_dshl(d, MBITS_BN254 / 2);
    ...
    BIG_add(r, r, b);
    BIG_norm(r);
    BIG_shl(t, MBITS_BN254 / 2);

    BIG_add(r, r, t);

    carry = r[NLEN_B256_28 - 1] >> TBITS_BN254;

    r[NLEN_B256_28 - 1] &= TMASK_BN254;
    r[0] += carry;

    r[224 / BASEBITS_B256_28] += carry << (224 % BASEBITS_B256_28); /* need to check that this falls mid-word */
    BIG_norm(r);

/*

    BIG t, b, t2, b2;
    int BTset = MBITS_BN254 / 2;
    chunk carry;
    BIG_split(t, b, d, MBITS_BN254);
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B256_28 - 1] >> TBITS_BN254; // + (carry<<(BASEBITS_B256_28-TBITS_BN254));
    r[NLEN_B256_28 - 1] &= TMASK_BN254;
...
/* Swap a and b if d=1 */
void BN254::FP_cswap(FP *a, FP *b, int d)
{
    sign32 t, c = d;
    BIG_cswap(a->g, b->g, d);
...
/* Move b to a if d=1 */
void BN254::FP_cmove(FP *a, FP *b, int d)
{
    sign32 c = -d;
...
void BN254::FP_fromBytes(FP *x,char *b)
{
    BIG t;
    BIG_fromBytes(t, b);
    FP_nres(x, t);
...
void BN254::FP_fpow(FP *r, FP *x)
{
    int i, j, k, bw, w, nw, lo, m, n, c, nd, e=PM1D2_BN254;
    FP xp[11], t, key;
...
void BN254::FP_sqrt(FP *r, FP *a, FP* h)
{
...
int BN254::FP_invsqrt(FP *i, FP *s, FP *x)
{
    FP h;
    int qr=FP_qr(x,&h);
    FP_sqrt(s,x,&h);
...
void BN254::FP_rand(FP *x,csprng *rng)
{
    BIG w,m;
    BIG_rcopy(m,Modulus);
