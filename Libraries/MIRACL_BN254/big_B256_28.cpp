...
/* Swap a and b if d=1 */
chunk B256_28::BIG_cswap(BIG a, BIG b, int d)
{
    int i;
...
/* Move g to f if d=1 */
chunk B256_28::BIG_cmove(BIG f, BIG g, int d)
{
    int i;
...
/* Move g to f if d=1 */
chunk B256_28::BIG_dcmove(DBIG f, DBIG g, int d)
{
    int i;
    chunk e,w,r,ra,t,b = (chunk) - d;
    w=0;
...
// bd is Max number of bits in b - Actual number of bits in m
void B256_28::BIG_ctddiv(BIG a,DBIG b,BIG m,int bd)
{
    int d,k=bd;
    DBIG c,dr;
    BIG e,r;
...
/* SU= 136 */
void B256_28::BIG_ddiv(BIG a, DBIG b, BIG m)
{
    int k=BIG_dnbits(b)-BIG_nbits(m);
    if (k<0) k=0;
    BIG_ctddiv(a,b,m,k);
...
// bd is Max number of bits in b - Actual number of bits in m
void B256_28::BIG_ctsdiv(BIG b,BIG m,int bd)
{
    int d, k=bd;
    BIG e,a,r,c;
    BIG_norm(b);
...
void B256_28::BIG_sdiv(BIG b, BIG m)
{
    int k=BIG_nbits(b)-BIG_nbits(m);
    if (k<0) k=0;
    BIG_ctsdiv(b,m,k);
...
/* a=1/a mod 2^BIGBITS_B256_28. This is very fast! */
void B256_28::BIG_invmod2m(BIG a)
{
    int i;
    BIG U, t1, b, c;
    BIG_zero(U);
    BIG_inc(U, invmod256(BIG_lastbits(a, 8)));
    for (i = 8; i < BIGBITS_B256_28; i <<= 1)
    {
        BIG_norm(U);
        BIG_copy(b, a);
