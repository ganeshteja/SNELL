...
#ifndef CORE_ARDUINO
static FP4 G2_TAB[G2_TABLE_BN254];  // space for precomputation on fixed G2 parameter
#endif
...
void BN254::BLS_HASH_TO_POINT(ECP *P, octet *M)
{
    FP u[2];
    ECP P1;
    char dst[50];
...
int BN254::BLS_KEY_PAIR_GENERATE(octet *IKM, octet* S, octet *W)
{
    int nbr,L;
    BIG r,s;
    DBIG dx;
...
int BN254::BLS_CORE_SIGN(octet *SIG, octet *M, octet *S)
{
    BIG s;
    ECP D;
...
int BN254::BLS_CORE_VERIFY(octet *SIG, octet *M, octet *W)
{
    FP12 v;
    ECP2 G, PK;
    ECP D, HM;
...
#ifndef CORE_ARDUINO
    FP12 r[ATE_BITS_BN254];  // space for accumulated line functions
