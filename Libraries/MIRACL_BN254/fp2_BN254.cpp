...
/* Move g to f if d=1 */
void BN254::FP2_cmove(FP2 *f, FP2 *g, int d)
{
    FP_cmove(&(f->a), &(g->a), d);
    FP_cmove(&(f->b), &(g->b), d);
}
...
{
    FP_toBytes(b,&(x->b));
    FP_toBytes(&b[MODBYTES_B256_28],&(x->a));
}

void BN254::FP2_fromBytes(FP2 *x,char *b)
{
    FP_fromBytes(&(x->b),b);
...
/* SU= 16 */
void BN254::FP2_from_BIGs(FP2 *w, BIG x, BIG y)
{
    FP_nres(&(w->a), x);
    FP_nres(&(w->b), y);
}
...
/* Set w=conj(x) */
/* SU= 16 */
void BN254::FP2_conj(FP2 *w, FP2 *x)
{
    FP_copy(&(w->a), &(x->a));
    FP_neg(&(w->b), &(x->b));
    FP_norm(&(w->b));
...
/* SU= 16 */
void BN254::FP2_pmul(FP2 *w, FP2 *x, FP *s)
{
    FP_mul(&(w->a), &(x->a), s);
    FP_mul(&(w->b), &(x->b), s);
}
...
/* SU= 8 */
void BN254::FP2_rawoutput(FP2 *w)
{
    printf("[");
    BIG_rawoutput(w->a.g);
    printf(",");
...
/* Set w=x/2 */
/* SU= 16 */
void BN254::FP2_div2(FP2 *w, FP2 *x)
{
    FP_div2(&(w->a), &(x->a));
    FP_div2(&(w->b), &(x->b));
}
...
void BN254::FP2_div_ip(FP2 *w)
{
    FP2 z;
    FP2_norm(w);
    FP2_from_ints(&z, (1 << QNRI_BN254), 1);
    FP2_inv(&z, &z, NULL);
    FP2_mul(w, &z, w);
#if TOWER_BN254 == POSITOWER
    FP2_neg(w, w);  // ***
...
/* SU= 208 */
/*
void BN254::FP2_pow(FP2 *r, FP2* a, BIG b)
{
    FP2 w;
    FP one;
    BIG z, zilch;
...
void BN254::FP2_sqrt(FP2 *w, FP2 *u, FP *h)
{
    FP w1, w2, w3, w4, hint;
    FP2 nw;
    int sgn,qr;
...
void BN254::FP2_rand(FP2 *x,csprng *rng)
{
    FP_rand(&(x->a),rng);
    FP_rand(&(x->b),rng);
}
